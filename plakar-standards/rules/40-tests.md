...
- Test the behaviour that was agreed, not the implementation in front of you. A
  test written by reading the code asserts what it already does, passes for as
  long as the bug lives, and has to be rewritten by anyone who touches the
  internals.
- Assert through the exported surface, on inputs and outputs. Reaching for an
  ...
  mechanism.
- If nobody has said what the code should do, that is the thing to settle
  first. Say so instead of inferring the contract from the code.
